...
class Client():
    ...
    def __init__(self,MainWindow):
        ...
        try:
            ...
            self.status = True
            self.MainFrame.label_2.setText('Successed')
            # 开启监听
    ...
    def updateCornersList(self):
        self.MainFrame.listWidget_2.clear() # 英语角列表
        for corner in self.corners:
            title = '{:<10s}{:>10s}'.format(corner['name'],corner['langue'])
    ...
    def enterRoom(self,text):
        ...
        if self.udpSocket != None:
            if name != self.current_corner_name:
                reply = QMessageBox.question(self.MainWindow, '提示', 'You need to quit Room {0} first. Are you sure?'.format(self.current_corner_name),
                                             QMessageBox.Yes, QMessageBox.No)
                if reply == QMessageBox.Yes:
                    print('Quit Room: ' + name)
                    self.MainFrame.listWidget.clear() # 清空消息列表
                    message = 'QUIT {name} {ip} {port}'.format(name= self.current_corner_name, ip=self.localIP, port=self.udpPort)
                    ...
                    return self.room
        self.MainFrame.groupBox_2.setTitle('Romm:' + name)
        if name != self.current_corner_name:
    ...
    def exitAPP(self):
        ...
        if self.tcpSocket != None:
            message = 'EXIT {name} {ip} {port}'.format(name = self.current_corner_name, ip= self.localIP, port= self.udpPort)
            self.tcpSocket.sendall(message.encode('utf-8'))
        # self.tcpSocket.shutdown(socket.SHUT_RDWR)
        # self.tcpSocket.close()
...
class ClientWindowDlg(QMainWindow):
    ...
    def enterRoom(self,item):
        corner = item.text()
        self.room = self.client.enterRoom(corner)
